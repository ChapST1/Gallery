...
export class API {
    ...
    verify(obj) {
        let view = obj.results ? this.showDataSearch(obj) : this.showDataRandom(obj);
    }
    ...
    async usersPhotos(url) {
        ...
        json.forEach(element => {
            ...
            <div class="img-container">
                ...
                <img src="${element.urls.regular}" class="user__images" data-user="${element.user.links.photos}" data-avatar="${element.user.profile_image.small}" data-name="${element.user.first_name}" data-lastname="${element.user.last_name}" data-likes="${element.likes}" data-bio="${element.user.bio}" data-views="${Math.round(Math.random() * 10000)
    ...
    modalImage(images) {
        ...
        images.forEach((e) => {
            e.addEventListener('click', () => {
                ...
                const avatar = document.querySelector('.user__avatar').setAttribute('src', e.getAttribute('data-avatar'));
                const name = document.querySelector('.user__name').textContent = `${e.getAttribute('data-name')} ${e.getAttribute('data-lastname')}`;
                const bio = document.querySelector('.description__bio').textContent = e.getAttribute('data-bio') == "null" ? "No biography :(" : e.getAttribute('data-bio');
                const downloadImage = document.querySelector('.user-downloads').setAttribute('href', e.getAttribute('data-image'))
                const likes = document.querySelector('.user__likes-number').textContent = e.getAttribute('data-likes')
                const time = document.querySelector('.description__time').textContent = e.getAttribute('data-time');
                const nameUser = document.querySelector('.user__photos-name').textContent = `More photos of ${e.getAttribute('data-name')} ${e.getAttribute('data-lastname')}`;
                const location = document.querySelector('.location__text').textContent = e.getAttribute('data-location') == "null" ? "My House" : e.getAttribute('data-location') || "My Library";
                const view = document.querySelector('.views').textContent = e.getAttribute('data-views');

                //access key
                let access = 'K-rbAXsWHZp9Wn_BpIkdDYESaR3Et_99ql4TFohhn-4';
                let getphotosUser = `${e.getAttribute('data-user')}?client_id=${access}`
                this.usersPhotos(getphotosUser)

                // activar el modal
                modal.classList.add('modal-active');
                modal.firstElementChild.firstElementChild.firstElementChild.setAttribute('src', e.getAttribute('src'))
                //desactivar el modal
